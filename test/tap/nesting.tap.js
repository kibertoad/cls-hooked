...
test('nested contexts on a single namespace', function (t) {
  ...
  namespace.run(function () {
    ...
    t.equal(namespace.get('value'), 1, 'namespaces have associated data even without contexts.')
    ...
    namespace.run(function () {
      ...
      t.equal(namespace.get('value'), 2, 'should revert to value set in top-level context')
    })
